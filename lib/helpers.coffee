Helpers = module.exports = load: (BABYLON) -> (->
  ...
  @Shapes =
    ...
    Sphere: 
      create: (name, subdivisions, size, scene) ->
        BABYLON.Mesh.CreateSphere name, subdivisions, size, scene
    Ground: 
      create: (name, width, depth, subdivisions, scene) ->
        BABYLON.Mesh.CreateGround name, width, depth, subdivisions, scene
